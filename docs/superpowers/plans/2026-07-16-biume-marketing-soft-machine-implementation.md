  ...
  return (
    <section data-landing-section="control" className="bg-[color:var(--machine-violet)] px-4 py-16 text-white sm:px-6 md:py-24 lg:px-8">
      <div className="mx-auto grid max-w-[90rem] gap-10 lg:grid-cols-[1fr_1.15fr] lg:items-end">
        ...
        <ul className="grid gap-3 sm:grid-cols-2">{decisions.map((decision, index) => <li key={decision} className="flex min-h-16 items-center gap-3 rounded-[var(--machine-control-radius)] bg-white/[0.12] px-4"><span aria-hidden="true" className="font-mono text-xs text-white/60">0{index + 1}</span><span className="font-semibold">{decision}</span></li>)}</ul>
      </div>
    </section>
  );
}
