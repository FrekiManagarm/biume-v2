            ...
            <TabsList aria-label={TABS_TITLE} className="flex flex-wrap gap-2 bg-transparent p-0">
              {SPECIMEN_STEPS.map((step) => (
                <TabsTrigger
                  key={step.id}
                  value={step.id}
                  className="min-h-11 rounded-full px-5 text-sm font-semibold text-[rgba(253,253,251,.62)] data-active:bg-[color:var(--lv5-violet)] data-active:text-white not-data-active:bg-[rgba(253,253,251,.08)]"
                >
                  {step.label}
                </TabsTrigger>
              ))}
