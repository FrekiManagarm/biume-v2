...
import type { getSession } from "#/functions/auth.function";
import type { getOrganizationSettings } from "#/functions/organization.function";
import {
  toInvoiceHistoryRows,
  type AutumnInvoice,
} from "#/lib/billing/invoice-history";
import { updateOrganization } from "#/lib/api/actions/organization.mutations";
import { updateUserNotifications } from "#/lib/api/actions/user.mutations";
...
type Organization = Awaited<ReturnType<typeof getOrganizationSettings>>;
 ...
 * `settingsSearchSchema` (`validateSearch`), partagé entre le loader et le
 * composant. Ici, aucune lecture serveur ne dépend de `tab`/`blocked`
 * (contrairement à `clients-view.tsx`/`patients-view.tsx`/
 * `reports-view.tsx`, où le préchargement react-query en a besoin) : cette
 * validation n'a donc besoin d'être reproduite que côté client, via
 * `useSearchParams()` — l'équivalent de `Route.useSearch()` pour un composant
 * client (voir table de correspondance de la tâche).
 */
function readSettingsSearch(searchParams: URLSearchParams) {
  const rawBlocked = searchParams.get("blocked");

  const parsed = settingsSearchSchema.safeParse({
    tab: searchParams.get("tab") ?? undefined,
    blocked: rawBlocked === null ? undefined : rawBlocked === "true",
  });

  return parsed.success ? parsed.data : {};
}

export function SettingsView({ session, organization }: SettingsViewProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const search = readSettingsSearch(searchParams);
  ...
  const organizationForm = useForm({
    ...
    onSubmit: async ({ value }) => {
      let result: Awaited<ReturnType<typeof updateOrganization>>;

      // `toActionResult` ne rejette plus pour une erreur applicative (elle
      // résout en { success: false }, déballée ci-dessous) : sans ce
      // déballage, un échec afficherait quand même « Entreprise mise à
      // jour. ». Un échec de transport (réseau coupé, serveur injoignable)
      // rejette en revanche toujours la promesse avant même d'atteindre ce
      // contrat — d'où ce filet, voir règle 5 et
      // select-organization-view.tsx.
      try {
        result = await updateOrganization(
          organizationSettingsSchema.parse(value),
        );
      } catch (error) {
        toast.error(
          error instanceof Error
            ? error.message
            : "Impossible de mettre à jour l'entreprise pour le moment.",
        );
        return;
      }
      ...
      // `router.invalidate()` (TanStack) relançait le `loader` de la route
      // (session + organisation) sans recharger la page. `router.refresh()`
      // (Next) est l'équivalent : il relance `page.tsx` (Server Component)
      // côté serveur et redescend `organization`/`session` à jour en props,
      // sans perdre l'état client de ce composant (onglet actif, etc.).
      router.refresh();
  ...
  const notificationForm = useForm({
    ...
    onSubmit: async ({ value }) => {
      let result: Awaited<ReturnType<typeof updateUserNotifications>>;

      // Même déballage que `organizationForm.onSubmit` ci-dessus, et pour la
      // même raison : sans lui, un échec afficherait « Préférences
      // enregistrées. ».
      try {
        result = await updateUserNotifications(
