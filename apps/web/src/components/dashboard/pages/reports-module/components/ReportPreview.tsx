...
export function ReportPreview({
  ...
  const staticObservations = observations.filter(
    (obs) => obs.type === "static",
  );
  const dynamicObservations = observations.filter(
    (obs) => obs.type === "dynamic",
  );

  // Regrouper les dysfonctions et suspicions issues du nouvel onglet
  const confirmedAnatomicalDysfunctions = anatomicalIssues.filter(
    (issue) => issue.type === "dysfunction",
  );
  const anatomicalSuspicions = anatomicalIssues.filter(
  ...
  const renderObservations = (obs: Observation[]) => {
    if (obs.length === 0)
      return <p className="text-muted-foreground italic">Aucune observation</p>;

    return obs.map((observation) => (
      <div key={observation.id} className="mb-3">
        <div className="flex items-center gap-2 flex-wrap">
          <div
            className={`w-2 h-2 rounded-full ${getSeverityColor(observation.severity)}`}
          />
          <span className="font-medium">{observation.region}</span>
          {observation.interventionZone && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-purple-100 text-purple-800">
              {
                interventionZones.find(
                  (z) => z.value === observation.interventionZone,
                )?.label
              }
            </span>
          )}
          ...
          <span className="text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-800">
            {observation.type === "static" ? "Statique" : "Dynamique"}
          </span>
        </div>
  ...
  const renderAnatomicalIssues = (issues: AnatomicalIssue[]) => {
    if (issues.length === 0)
      return (
        <p className="text-muted-foreground italic">Aucun élément à afficher</p>
      );
    ...
    return issues.map((issue) => (
      <div key={issue.id} className="mb-3">
        <div className="flex items-center gap-2 flex-wrap">
          <div
            className={`w-2 h-2 rounded-full ${getSeverityColor(issue.severity)}`}
          />
          <span className="font-medium">
            {issue.anatomicalPart?.name || "Région inconnue"}
          </span>
          ...
          <span className="text-xs text-muted-foreground">
            ({issue.type === "dysfunction" ? "Sévérité" : "Indice"}:{" "}
        ...
        <div className="space-y-6 py-4">
          ...
          <div>
            <h3 className="text-lg font-medium mb-2">
              {observationTypes.find((t) => t.value === "static")?.label ||
                "Observations statiques"}
            </h3>
          ...
          <div>
            <h3 className="text-lg font-medium mb-2">
              {observationTypes.find((t) => t.value === "dynamic")?.label ||
                "Observations dynamiques"}
            </h3>
            <Separator className="mb-3" />
